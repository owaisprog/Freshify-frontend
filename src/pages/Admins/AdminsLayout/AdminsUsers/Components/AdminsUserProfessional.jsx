...
function AdminsUserProfessional({ isLoading, error }) {
  ...
  // ✅ Fetch services
  const { data: users = [] } = useQueryHook({
    queryKey: ["users", id], // ✅ Cache users by owner ID
    endpoint: `/api/get-users`,
    staleTime: 0 * 60 * 1000, // Cache for 15 minutes
  });
  const userdata = users.filter((val) => val?.location?._id === location?._id);
...
export default AdminsUserProfessional;
